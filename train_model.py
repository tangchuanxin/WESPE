...
with tf.Graph().as_default(), tf.Session() as sess:
    ...
    adversarial_color_ = tf.multiply(enhanced_blur, 1 - adv_color_) + tf.multiply(dslr_blur, adv_color_)
    adversarial_color_image = tf.reshape(adversarial_color_, [-1, PATCH_HEIGHT, PATCH_WIDTH, 3])

    discrim_predictions = models.adversarial(adversarial_image)
    discrim_predictions_color = models.adversarial(adversarial_color_image)
    ...
    phone_gen_vgg = vgg.net(vgg_dir, vgg.preprocess(phone_image_gen * 255))
    ...
    loss_content = 2 * tf.nn.l2_loss(phone_vgg[CONTENT_LAYER] - phone_gen_vgg[CONTENT_LAYER]) / content_size

    # 3) color loss

    discrim_target_color = tf.concat([adv_color_, 1 - adv_color_], 1)
    loss_discrim_color = -tf.reduce_sum(discrim_target_color * tf.log(tf.clip_by_value(discrim_predictions_color, 1e-10, 1.0)))
    loss_color = -loss_discrim_color 
    correct_predictions_color = tf.equal(tf.argmax(discrim_predictions_color, 1), tf.argmax(discrim_target_color, 1))
    discim_accuracy_color = tf.reduce_mean(tf.cast(correct_predictions_color, tf.float32))
    ...
    train_step_disc = tf.train.AdamOptimizer(learning_rate).minimize(loss_discrim+loss_discrim_color, var_list=discriminator_vars)
    ...
    for i in range(num_train_iters):
        ...
        [loss_temp, temp] = sess.run([loss_generator, train_step_gen],
                                        feed_dict={phone_: phone_images, dslr_: dslr_images, adv_: all_zeros, adv_color_: all_zeros})
        ...
        swaps_color = np.reshape(np.random.randint(0, 2, batch_size), [batch_size, 1])

        phone_images = train_data[idx_train]
        dslr_images = train_answ[idx_train]

        [accuracy_temp, temp] = sess.run([discim_accuracy, train_step_disc],
                                        feed_dict={phone_: phone_images, dslr_: dslr_images, adv_: swaps, adv_color_: swaps_color })
